...
def extract(page):
    ...
    url = f"https://uk.indeed.com/jobs?q=blockchain%20developer&l=London%2C%20Greater%20London&start={page}"
...
def findMaxPage(soup):
    pageNumberSplit1 = (
        soup.find("div", {"id": "searchCountPages"}).text.strip().split("of")
    )
...
a = extract(0)
print(findMaxPage(a))
for i in range(0, findMaxPage(a)):
    c = extract(i * 10)
    transform(c)
    sleep(randint(1, 20))
